	...
	<?= form_open() ?>
		<div class="col-md-6">
			...
			<div class="form-group">
				...
				<?= form_input('company_name', set_value('company_name', @$rs->company_name ? $rs->company_name : ''), 'class="form-control"') ?>
				<?= form_error('company_name') ?>
			</div>
			<div class="form-group">
				<?= lang('office_location', 'office_location') ?>
				<?= form_input('office_location', set_value('office_location', @$rs->office_location ? $rs->office_location : ''), 'class="form-control"') ?>
		...
		<div class="form-group">
			<?= form_button([
				...
				<a href="<?= site_url('generalconsumer'); ?>" type="button" class="btn btn-danger pull-right">Cancel</a>
		</div>
	<?= form_close() ?>
</div>
</div>
